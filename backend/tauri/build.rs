...
fn main() {
    ...
    let raw = fs::read_to_string(daemon_manifest).expect("read backend/tauri/daemon/Cargo.toml");
    let parsed: toml::Value = toml::from_str(&raw).expect("parse backend/tauri/daemon/Cargo.toml");
    let version = parsed
        .get("package")
