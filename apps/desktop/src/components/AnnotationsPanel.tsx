  ...
  if (!hasAnnotations) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground/50 gap-2 px-4">
        ...
        <span className="text-[11px] text-center">Click on lines to add annotations</span>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-1 px-3 py-2 border-b border-border shrink-0">
        ...
        {hasUnresolved && (
          <button
            ...
            className="px-2 py-0.5 rounded text-[10px] text-blue-400 hover:text-blue-300 hover:bg-blue-500/20 ml-auto"
          >
            Send to Claude
          </button>
        )}
        ...
        {scoped.length === 0 ? (
          ...
          <div className="flex flex-col">
            {[...grouped.entries()].map(([filePath, fileAnnotations]) => (
              <div key={filePath}>
                <div className="px-3 py-1.5 text-[9px] uppercase tracking-[1.2px] text-muted-foreground/60 font-semibold border-b border-border/50 font-mono normal-case text-[10px] truncate">
                  {filePath.split("/").pop()}
                </div>
                <div className="flex flex-col gap-1.5 p-2">
