...
export function ThemeCard({ theme, isActive, onSelect, onDelete }: ThemeCardProps) {
  ...
  return (
    // The delete control is a sibling of the selection button, never a
    // descendant of it — interactive content inside a <button> is invalid and
    // the outer target swallows activation.
    <div
      className="group/card relative rounded-lg overflow-hidden"
      style={{
    ...
    >
      <button
        ...
        className="block w-full text-left outline-none transition-all"
      >
        {/* Mini UI preview */}
        ...
        <div
          style={{
            // Reserve room on the right so a long theme name doesn't run under
            // the absolutely-positioned delete control.
            padding: onDelete ? "6px 24px 6px 8px" : "6px 8px",
      ...
      {onDelete && (
        <button
          ...
          className="absolute bottom-1 right-1.5 flex size-5 items-center justify-center rounded opacity-0 outline-none transition-opacity group-hover/card:opacity-100 focus:opacity-100"
